...
if [ -z "$JAVA_OPTS" ]; then
  export JAVA_OPTS="-server -XX:MaxPermSize=128m -Xms512m -Xmx512m -XX:+UseParallelOldGC -XX:NewRatio=2"
fi 
...
export CLUSTER_CONFIG_DIR="/opt/geoserver/data_dir/cluster/instance_$randomString"
...
exec "$_RUNJAVA" $JAVA_OPTS -DCLUSTER_CONFIG_DIR="$CLUSTER_CONFIG_DIR" -Dactivemq.base="$CLUSTER_CONFIG_DIR/tmp" -Dactivemq.transportConnectors.server.uri="tcp://0.0.0.0:0?maximumConnections=1000&wireFormat.maxFrameSize=104857600&jms.useAsyncSend=true&transport.daemon=true&trace=true" -DGEOSERVER_DATA_DIR="$GEOSERVER_DATA_DIR" -Djava.awt.headless=true -DSTOP.PORT=8079 -DSTOP.KEY=geoserver -jar start.jar 
